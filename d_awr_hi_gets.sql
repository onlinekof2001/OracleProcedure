...
--	&2 date fin
--      &3 level 
--       3 pas d'entete, pas de texte de requete , uniquement les stat par sqlid
--	     2 entete + stat 
-- 	     1 stat + text
--         pas précisé : tout.
...
-- je recupere le nombre de logical read de toute l'instance sur l'intervale
set termout off;
...
set underline off;
--set echo on
col aa format a1024 heading '                                                    CPU     Elapsed| Buffer Gets    Executions   Gets per Exec  %Total Time (s) Time (s)  Sql_Id        ToTal B Get     begin snap time   end snap time|--------------- ------------ -------------- ------ -------- --------- ------------- --------------- ----------------- -----------------' 
col sqltext heading ''
column hv noprint;
column tbg noprint;
...
select *
  from ( 
  select   lpad(to_char(nvl(b.buffer_gets,0) ,'99999999999'   )
                        ,15) ||' '||
                    lpad(to_char(nvl(b.executions,0)  ,'999999999')
                        ,12) ||' '||            
                    lpad(to_char(nvl(decode(b.executions,0,-1,b.buffer_gets /b.executions),0) ,'9999999999')
                        ,14) ||' '||
                    lpad(to_char(nvl(100*(b.buffer_gets/:gets),0),'990.0')
                        , 6) ||' '||
         ...
         from 
              ...
              FROM   (SELECT 
                          ...
                          group by sql_id
                          ORDER  BY SUM(b.buffer_gets_delta) DESC
                      )
              WHERE  buffer_gets is not null and  rownum <= :nbreq
              ) b
            , dba_hist_sqltext     st 
        where st.dbid              = :dbid
...
-- :silent  =1
-- :nbreq = 5
-- permet de mettre des espaces entre chaque query
break on hv skip 1; 

set lines 140
select *
  from ( 
  select   lpad(to_char(b.buffer_gets ,'99,999,999,999,999'   )
                        ,15) ||' '||
                    lpad(to_char(b.executions  ,'999,999,999')
                        ,12) ||' '||            
                    lpad(to_char(decode(b.executions,0,-1,b.buffer_gets /b.executions)  ,'9,999,999,999')
                        ,14) ||' '||
                    lpad(to_char(100*(b.buffer_gets/:gets),'990.0')
                        , 6) ||' '||
                    lpad(to_char(b.cpu_time/1000000, '99990.00')
                        , 8) || ' ' ||
                    lpad(to_char(b.elapsed_time/1000000, '999990.00')
                    ...
                    lpad(b.sql_id
                        ,13) ||' '|| 
                    lpad(to_char(:gets,'9,999,999,999')
                        ,15) aa
            , replace(dbms_lob.substr(sql_text,3950,1),chr(9),' ') sqltext
            , b.sql_id hv
         from 
              (sELECT  sql_id, buffer_gets, executions, elapsed_time, CPU_TIME
              FROM   (SELECT 
