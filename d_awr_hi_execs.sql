...
column executions     new_value executions format 99999999999999999999990 heading 'executions';
SELECT d.NAME DATABASE,
       bs.dbid,
       bs.instance_number,
       bs.snap_id bsnap_id,
       es.snap_id esnap_id,
       es.value-bs.value   executions
FROM   v$database d,
       DBA_HIST_SYSSTAT bs,
       DBA_HIST_SYSSTAT es
WHERE  
       ...
       AND (es.snap_id = :e_snap_id AND es.dbid = &&dbid  AND es.instance_number = :inst_num)
       AND (bs.stat_id = es.stat_id AND bs.stat_name = es.stat_name)
       and bs.stat_name ='execute count'
        ;
...
set termout on        
--select 'post calcul gets' , :gets, :b_snap_id, :e_snap_id from dual;
...
set underline off;
--set echo on
col aa format a1024 heading '                                             Exec   CPU     Elapsed|    Executions   Buffer Gets  Gets per Exec  %Total Time (s) Time (s)  Sql_Id            ToTal Exec |------------ --------------- -------------- ------ -------- --------- ------------- ---------------' 
col sqltext heading ''
column hv noprint;
column tbg noprint;
...
select *
  ...
  select   
                    lpad(to_char(b.executions  ,'999,999,999')
                        ,12) ||' '||    
                    lpad(to_char(b.buffer_gets ,'99,999,999,999'   )
                        ,15) ||' '||
                    lpad(to_char(b.buffer_gets /b.executions ,'999,999,990.0')
                        ,14) ||' '||
                    lpad(to_char(100*(b.executions/:executions),'990.0')
                        , 6) ||' '||
                    ...
                    lpad(b.sql_id
                        ,13) ||' '|| 
                    lpad(to_char(:executions,'99,999,999,999')  
                        ,15) aa
            , b.sql_id hv
         from 
              ...
              FROM   (SELECT 
                          ...
                          group by sql_id
                          ORDER  BY SUM(b.executions_delta) DESC
                      )
              WHERE  rownum <= :nbreq
              ) b
            , dba_hist_sqltext     st 
...
prompt 'les 5 requetes les plus consommatrice - hight-exec'
-- permet de mettre des espaces entre chaque query
break on hv skip 1; 
set lines 105
select *
  from ( 
  select    lpad(to_char(b.executions  ,'999,999,999')
                        ,12) ||' '||    
                    lpad(to_char(b.buffer_gets ,'99,999,999,999'   )
                        ,15) ||' '||
                    lpad(to_char(b.buffer_gets /b.executions ,'999,999,990.0')
